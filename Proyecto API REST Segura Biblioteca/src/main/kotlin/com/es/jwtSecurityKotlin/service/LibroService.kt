...
class LibroService {
    ...
    fun createLibro(libro: Libro): Libro {
        ...
        val titulo = libro.titulo ?: throw BadRequestException("El titulo no puede ser nulo.")
        val editorial = libro.editorial ?: throw BadRequestException("La editorial no puede ser nula.")
        ...
        if (libroRepository.findByTitulo(libro.titulo!!).isPresent && libroRepository.findByEditorial(libro.editorial!!).isPresent) {
            throw ConflictException("El Libro con titulo ${libro.titulo} ya existe con esa version de la editorial.")
        }
    ...
    fun getLibroById(id: Long): Libro {
        return libroRepository.findById(id)
            .orElseThrow { NotFoundException("No se encontró un libro con el id: $id") }
    }
